...
def create_simulation(scenario, universe_class, scenario_param = None, threads = 1):
	...
	if scenario == 'solarsystem':
		...
		create_galaxy(
			...
			r = [0., 0., 0.], v = [0., 0., 0.], g_alpha = 0., g_beta = 0.,
			m_hole = 4e40, m_star = 2e30, radius = 1e20
			)
	else:
...
def create_solarsystem(universe_obj):
	...
	universe_obj.add_object(
		name = 'earth',
		r = [0., -149597870700.0, 0.], v = [29777.777, 0., 0.],
		m = 5.97237e24
		)
	...
	for n in range(stars_len):
		...
		if n < (stars_len * 4 // 5):
			...
			r_s = [
				...
				(0.5 * random.random() - 0.25) * radius * ((4.5 + 0.1) * radius - r_abs) / ((4.5 + 0.1) * radius)
				]
		...
		v_alpha = alpha - (math.pi / 2) # Phasenverschiebung [Drehung] um -90° (von ausgehend 0 bis 360°): Geschwindigkeitsvektor im rechten Winkel zur Bahn
...
def load_simulation(universe_class, fn, gn, threads = None):
	...
	param = {
		'%s' % attr: dg.attrs[attr] for attr in dg.attrs.keys()
		}
