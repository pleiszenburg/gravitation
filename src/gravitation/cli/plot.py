...
@click.option(
	'--logfile', '-l',
	default = 'benchmark.json', type = click.File('r'), show_default = True,
	multiple = True, required = True,
	help = 'name of input log file, can be specified multiple times',
	)
@click.option(
	'--html_out', '-o',
	default = 'benchmark.html', type = click.Path(
...
def plot(logfile, html_out):
	...
	data_dict = {item: dict() for item in {
		item['meta']['simulation']['kernel'] for item in data_list
		}}

	for item in data_list:
		data_dict[
			item['meta']['simulation']['kernel']
			][
	...
	for kernel_name, kernel_results in sorted(data_dict.items(), key = lambda x: x[0]):
		...
		traces.append(
			go.Scatter(x = x, y = y, name = kernel_name, mode = 'lines+markers')
			)
