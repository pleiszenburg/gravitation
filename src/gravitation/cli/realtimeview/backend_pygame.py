...
class realtimeview():
	...
	def __init__(self,
		kernel, threads, scenario, scenario_param, steps_per_frame = None, max_iterations = None,
		):
		...
		self._init_canvas(
			spf = self._universe._screen['steps_per_frame'] if steps_per_frame is None else steps_per_frame,
	...
	def _draw_text(self, text, pos, right = False):
		...
		if right:
			self._canvas.blit(
				rendered_text,
				(pos[0] - rendered_text.get_width(), pos[1])
				)
		else:
	...
	def _loop_canvas(self):
		...
		self._draw_text(
			'%.02f F/s' % (1.e9 / self._timer_fps.avg()),
			(self._pixel_size[0], 0), right = True
			)
		self._draw_text(
			'%.02f S/s' % (1.e9 / self._timer_sps.avg()),
			(self._pixel_size[0], self._font_size), right = True
			)
		self._draw_text(
			'%.01e s/S' % (self._timer_sps.avg() / 1.e9),
			(self._pixel_size[0], 2 * self._font_size), right = True
			)
		pygame.display.flip()
