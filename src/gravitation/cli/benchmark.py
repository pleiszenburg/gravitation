...
def _range(start, end):
	...
	l_intp.extend([
		x for y in zip(l[1:], [i + j for i, j in zip(l[1:], l[:-1])]) for x in y
		][:-1])
	for n in l_intp:
		yield n
...
@click.option(
	'--logfile', '-l',
	default = 'benchmark.log', type = str, show_default = True,
	help = 'name of output log file',
	)
@click.option(
	'--data_out_file', '-o',
	default = 'data.h5', type = str, show_default = True,
	help = 'name of output data file',
	)
@click.option(
	'--interpreter', '-i',
	default = 'python3', type = str, show_default = True,
	help = 'python interpreter command',
	)
@click.option(
	'--kernel', '-k',
	type = click.Choice(sorted(list(inventory.keys()))), multiple = True,
...
@click.option(
	'--n_body_power_boundaries', '-b',
	default = (2, 16), type = (int, int), show_default = True,
	help = '2^x bodies in simulation, for x from lower to upper boundary',
	)
@click.option(
	'--save_after_iteration', '-s',
	type = int, show_default = True, multiple = True,
	help = 'save model universe into file iteration x, -1 if nothing should be saved',
	)
@click.option(
	'--min_iterations', '-i',
	default = 10, type = int, show_default = True,
...
@click.option(
	'--display', '-d',
	default = 'plot', type = click.Choice(['plot', 'log', 'none']), show_default = True,
...
def benchmark(
	logfile, data_out_file, interpreter, kernel, all_kernels, n_body_power_boundaries,
	save_after_iteration, min_iterations, min_total_runtime, display, threads,
	):
	...
	results_dict = {kernel_name: {
		threads_num: dict() for threads_num in range(1, MAX_TREADS + 1)
	...
	for kernel_name in kernels:
		...
		for threads_num in threads_iterator:
			for bodies in _range(*n_body_power_boundaries):
				proc.run_command(
					worker_command(
						data_out_file, interpreter, kernel_name, 'galaxy', {'stars_len': bodies},
						save_after_iteration, min_iterations, min_total_runtime, threads_num,
						),
					unbuffer = True,
					processing = _process_data(
						kernel_name, threads_num, bodies,
						results_dict, outputlines_list, fh, display,
						),
					)
				fh.flush()
