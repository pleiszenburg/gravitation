...
def _parse_itemstr_to_itemdict(item_str):
	...
	size = copy.deepcopy([line_dict for line_dict in line_list if line_dict['log'] == 'SIZE'])
	if len(size) > 1:
...
@click.option(
	'--logfile', '-l',
	default = 'benchmark.log', type = click.File('r'), show_default = True,
	help = 'name of input log file',
	)
@click.option(
	'--data', '-o',
	default = 'benchmark.json', type = click.File('w'), show_default = True,
