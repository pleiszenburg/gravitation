...
class universe(universe_base):
	...
	def start_kernel(self):
		...
		self.cpu_pool = mp.Pool(
			...
			initargs = (mass_m_array, self._G, self.SIM_DIM, self.MASS_LEN, self.DTYPE,),
			)

	@staticmethod
	def update_pair(i, k, mass_r_array):
		np.subtract(mass_r_array[i,:], mass_r_array[i+1:,:], out = context['relative_r'][:k])
		np.multiply(context['relative_r'][:k], context['relative_r'][:k], out = context['distance_sqv'][:k])
		np.add.reduce(context['distance_sqv'][:k], axis = 1, out = context['distance_sq'][:k])
		np.sqrt(context['distance_sq'][:k], out = context['distance_inv'][:k])
		np.divide(1.0, context['distance_inv'][:k], out = context['distance_inv'][:k])
		np.multiply(context['relative_r'][:k], context['distance_inv'][:k].reshape(k, 1), out = context['relative_r'][:k])
		np.divide(context['G'], context['distance_sq'][:k], out = context['a_factor'][:k])
		np.multiply(context['a_factor'][:k], context['mass_m_array'][i+1:], out = context['a1'][:k])
		np.multiply(context['a_factor'][:k], context['mass_m_array'][i], out = context['a2'][:k])
		np.multiply(context['relative_r'][:k], context['a1'][:k].reshape(k, 1), out = context['a1r'][:k])
		np.add.reduce(context['a1r'][:k], axis = 0, out = context['a1v'])
		np.subtract(context['mass_a_array'][i,:], context['a1v'], out = context['mass_a_array'][i,:])
		np.multiply(context['relative_r'][:k], context['a2'][:k].reshape(k, 1), out = context['a2r'][:k])
		np.add(context['mass_a_array'][i+1:,:], context['a2r'][:k], out = context['mass_a_array'][i+1:,:])
	...
	def step_stage1_batch(i_start, i_end, mass_len, mass_r_array):
		...
		for row in range(i_start, i_end):
			universe.update_pair(row, mass_len - 1 - row, mass_r_array) # max for temp arrays
	...
	def step_stage1(self):
		...
		pool_results = [
			self.cpu_pool.apply_async(
				...
				args = (i_start, i_end, self.MASS_LEN, self.mass_r_array,)
