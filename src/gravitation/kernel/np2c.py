...
https://github.com/pleiszenburg/gravitation
	...
	src/gravitation/kernel/np2c.py: Kernel
...
__longname__ = "numpy-backend (2c)"
__version__ = "0.0.1"
__description__ = "numpy backend, optimized array layout, stage 2 also numpy, CPU-cache experiment (optimized)"
...
class universe(universe_base):
    ...
    def start_kernel(self):
        ...
        for col_min, col_max in self.COLS_LIST:
            for row in range(0, col_max - 1):
                ...
                b_z = col_max
                b_len = b_z - b_a
                self.ITER_LIST.append((row, b_a, b_z, b_len))
    ...
    def update_pair(self, item, b_a, b_z, b_len):
        np.subtract(
            self.mass_r_array[item, :],
            self.mass_r_array[b_a:b_z, :],
            out=self.relative_r[:b_len],
        )
        np.multiply(
            self.relative_r[:b_len],
            self.relative_r[:b_len],
            out=self.distance_sqv[:b_len],
        )
        ...
        np.multiply(
            self.relative_r[:b_len],
            self.distance_inv[:b_len].reshape(b_len, 1),
            out=self.relative_r[:b_len],
        )
        np.divide(self._G, self.distance_sq[:b_len], out=self.a_factor[:b_len])
        np.multiply(
            self.a_factor[:b_len], self.mass_m_array[b_a:b_z], out=self.a1[:b_len]
        )
        np.multiply(self.a_factor[:b_len], self.mass_m_array[item], out=self.a2[:b_len])
        np.multiply(
            self.relative_r[:b_len],
            self.a1[:b_len].reshape(b_len, 1),
            out=self.a1r[:b_len],
        )
        np.add.reduce(self.a1r[:b_len], axis=0, out=self.a1v)
        np.subtract(
            self.mass_a_array[item, :], self.a1v, out=self.mass_a_array[item, :]
        )
        np.multiply(
            self.relative_r[:b_len],
            self.a2[:b_len].reshape(b_len, 1),
            out=self.a2r[:b_len],
        )
        np.add(
            self.mass_a_array[b_a:b_z, :],
            self.a2r[:b_len],
            out=self.mass_a_array[b_a:b_z, :],
        )

    def step_stage1(self):
        # Run "pair" calculation: One object against vector of objects per iteration
        for param in self.ITER_LIST:
            self.update_pair(*param)
