...
class universe(universe_base):
	...
	def start_kernel(self):
		...
		self.distance_sqv = cp.zeros((self.MASS_LEN - 1, self.SIM_DIM), dtype = self.DTYPE)
	...
	def update_pair(self, i, k):
		cp.subtract(self.mass_r_arrayg[i,:], self.mass_r_arrayg[i+1:,:], out = self.relative_r[:k])
		...
		cp.multiply(self.relative_r[:k], self.distance_inv[:k].reshape(k, 1), out = self.relative_r[:k])
		...
		cp.add(self.mass_a_arrayg[i+1:,:], self.a2r[:k], out = self.mass_a_arrayg[i+1:,:])
