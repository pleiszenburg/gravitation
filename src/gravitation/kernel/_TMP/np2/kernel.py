...
__version__ = "0.0.2"
__description__ = "numpy backend, no memory allocations"
...
class Universe(UniverseBase):
    ...
    def _update_pair(self, i: int, k: int):
        ...
        np.multiply(
            self._relative_r[:k], self._relative_r[:k], out=self._distance_sqv[:k]
        )
        np.add.reduce(self._distance_sqv[:k], axis=1, out=self._distance_sq[:k])
        np.sqrt(self._distance_sq[:k], out=self._distance_inv[:k])
        np.divide(1.0, self._distance_inv[:k], out=self._distance_inv[:k])
        np.multiply(
            self._relative_r[:k],
            self._distance_inv[:k, None],
            out=self._relative_r[:k],
        )
        ...
        np.multiply(self._relative_r[:k], self._a1[:k, None], out=self._a1r[:k])
        np.add.reduce(self._a1r[:k], axis=0, out=self._a1v)
        np.subtract(self._a[i, :], self._a1v, out=self._a[i, :])
        np.multiply(self._relative_r[:k], self._a2[:k, None], out=self._a2r[:k])
        np.add(
            self._a[i + 1 :, :],
            self._a2r[:k],
            out=self._a[i + 1 :, :],
