...
https://github.com/pleiszenburg/gravitation
    ...
    src/gravitation/kernel/np3/kernel.py: Kernel
...
class Universe(UniverseBase):
    ...
    def start_kernel(self):
        ...
        self._relative_r = np.zeros((DIMS, len(self) - 1), dtype=self._dtype.name)
        self._distance_sq = np.zeros((len(self) - 1,), dtype=self._dtype.name)
        self._distance_sqv = np.zeros((DIMS, len(self) - 1), dtype=self._dtype.name)
        ...
        self._a1r = np.zeros((DIMS, len(self) - 1), dtype=self._dtype.name)
        self._a1v = np.zeros((DIMS,), dtype=self._dtype.name)
        self._a2 = np.zeros((len(self) - 1,), dtype=self._dtype.name)
        self._a2r = np.zeros((DIMS, len(self) - 1), dtype=self._dtype.name)
