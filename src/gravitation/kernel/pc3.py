...
__version__ = '0.0.1'
__description__ = 'pycuda, O(N*(N-1)) complexity, optimization experiment - UNSTABLE, BUGS!'
...
class universe(universe_base):
	...
	def start_kernel(self):
		...
		self.threads_per_block = 128 # 512 # http://recurial.com/wp-content/uploads/2016/02/p1-vollmer.pdf
		...
		self.sm = SourceModule(SM.format(
	...
	def step_stage1(self):
		...
		self.sm_update_all(
			self.mass_rx_array_g, self.mass_ry_array_g, self.mass_rz_array_g,
			self.mass_ax_array_g, self.mass_ay_array_g, self.mass_az_array_g,
			self.mass_m_array_g,
			block = (self.threads_per_block, 1, 1), grid = (self.blocks, 1)
			)
		# Copy data to GPU memory
