...
class universe(universe_base):
    def start_kernel(self):
        ...
        self.mass_r_arrayg = torch.zeros(
            (self.MASS_LEN, self.SIM_DIM), dtype=self.TDTYPE
        )
        self.mass_a_arrayc = np.zeros((self.MASS_LEN, self.SIM_DIM), dtype=self.DTYPE)
        self.mass_a_arrayg = torch.zeros(
            (self.MASS_LEN, self.SIM_DIM), dtype=self.TDTYPE
        )
        ...
        self.relative_r = torch.zeros(
            (self.MASS_LEN - 1, self.SIM_DIM), dtype=self.TDTYPE
        )
        self.distance_sq = torch.zeros((self.MASS_LEN - 1,), dtype=self.TDTYPE)
        self.distance_sqv = torch.zeros(
            (self.MASS_LEN - 1, self.SIM_DIM), dtype=self.TDTYPE
        )
    ...
    def update_pair(self, i, k):
        ...
        torch.mul(self.distance_sq[:k], 1.0 / self._G, out=self.a_factor[:k])
